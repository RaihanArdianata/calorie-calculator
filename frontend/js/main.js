...
$(document).ready(() => {
    ...
    $('#login-form').on('submit', (event) => {
        ...
        apiService.post('api/auth/login', bodyRequest)
            ...
            .fail((jqXHR, textStatus, errorThrown) => {
                hideLoader();
                if (jqXHR.status === 400) {
                    $('#message').removeClass('is-hidden').addClass('is-danger').text('Login failed. Invalid username or password.');
                } else {
                    $('#message').removeClass('is-hidden').addClass('is-danger').text('Login failed. Please try again.');
                }
            });
    });
    ...
    $('#register-form').on('submit', (event) => {
        ...
        if (password !== confirmPassword) {
            hideLoader();
            $('#message').removeClass('is-hidden').addClass('is-danger').text('Passwords does not match.');
            return;
        }
        ...
        // Send AJAX request
        apiService.post('api/auth/register', bodyRequest)
            .done(response => {
                hideLoader();
                $('#message').removeClass('is-hidden').addClass('is-success').text('Registration successful!');
                resetFormLogin()
            })
            .fail((jqXHR, textStatus, errorThrown) => {
                hideLoader();
                if (jqXHR.responseJSON && jqXHR.responseJSON.details) {
                    ...
                    $('#message').removeClass('is-hidden').removeClass('is-success').addClass('is-danger').html(errorMessage);
                } else {
                    const errorMessage = jqXHR.responseJSON ? jqXHR.responseJSON.error : 'Registration failed. Please try again.';
                    $('#message').removeClass('is-hidden').removeClass('is-success').addClass('is-danger').text(errorMessage);
                }
            });
    });
});
